...
/**
  * Useful concept stolen from Diode https://github.com/suzaku-io/diode
  */
...
sealed abstract class Pot[+A] extends Product with Serializable { self =>
  ...
  def pending(startTime: Long): Pot[A]
  ...
  @inline final def collect[B](pf: PartialFunction[A, B]): Pot[B] =
    if (!isEmpty) pf.lift(this.get).map(b => Ready(b)).getOrElse(Empty)
    else Empty
  ...
  /**
    ...
    */
  @nowarn("msg=parameter value f in method recoverWith is never used")
  ...
  /**
    ...
    */
  @nowarn("msg=parameter value f in method recover is never used")
...
object Pot {
  ...
  implicit object potMonad extends Monad[Pot] {
    override def pure[A](x: A): Pot[A] = Ready(x)
    ...
    override def tailRecM[A, B](a: A)(f: A => Pot[Either[A, B]]): Pot[B] =
      f(a) match {
        case Empty       => Empty
        case Unavailable => Unavailable
        case Failed(ex)  => Failed(ex)
        case FailedStale(Left(nextA), ex) =>
          tailRecM(nextA)(a =>
            f(a) match {
              case Empty    => Failed(ex)
              case Ready(x) => FailedStale(x, ex)
              case other    => other
            }
          )
        case FailedStale(Right(b), ex) => FailedStale(b, ex)
        case Pending(t)                => Pending(t)
        case PendingStale(Right(b), t) => PendingStale(b, t)
        case PendingStale(Left(nextA), t) =>
          tailRecM(nextA)(a =>
            f(a) match {
              case Empty    => Pending(t)
              case Ready(x) => PendingStale(x, t)
              case other    => other
            }
          )
        case Ready(Right(b))    => Ready(b)
        case Ready(Left(nextA)) => tailRecM(nextA)(f)
      }
  }

  implicit def eqPot[A: Eq]: Eq[Pot[A]] = Eq.fromUniversalEquals

  def currentTime: URIO[Clock, Long] =
    zio.clock.currentTime(TimeUnit.MILLISECONDS)

  implicit class ZioPotOps(private val obj: ZIO.type) {
    @inline def fromPot[A](pot: Pot[A]): IO[Throwable, A] = IO.effect(pot.get)
    @inline def fromPotM[R, A](pot: RIO[R, Pot[A]]): RIO[R, A] =
      pot.flatMap(ZIO.fromPot)
  }

  implicit class ZIOInstancePotOps[A](private val pot: Pot[A]) {
    @inline def supplyWith[R](action: RIO[R, A]): ZIO[R, Nothing, Pot[A]] =
      action.map(Ready.apply).catchAll(ex => UIO.succeed(pot.fail(ex)))
  }
}
...
case object Empty extends Pot[Nothing] {
  ...
  override def pending(startTime: Long): Pot[Nothing] =
    Pending(startTime)
  override def fail(exception: Throwable): Pot[Nothing] = Failed(exception)
}
...
final case class Ready[+A](x: A) extends Pot[A] {
  ...
  override def pending(startTime: Long): Pot[A] =
    PendingStale(x, startTime)
  override def fail(exception: Throwable): Pot[A] = FailedStale(x, exception)
}
...
sealed trait PendingBase {
  ...
  def isPending = true
  def isUnavailable = false
  def state: PotState = PotState.PotPending
  def duration(currentTime: Long): Int =
    (currentTime - startTime).toInt
}

final case class Pending(startTime: Long)
    extends Pot[Nothing]
    with PendingBase {
  ...
  override def pending(startTime: Long = startTime): Pot[Nothing] =
    copy(startTime)
  override def fail(exception: Throwable): Pot[Nothing] = Failed(exception)
}

object Pending {
  def make: URIO[Clock, Pending] =
    Pot.currentTime.map(Pending(_))
}

// TODO: use Zio clock to eliminate the current time side effect
final case class PendingStale[+A](x: A, startTime: Long)
    extends Pot[A]
    with PendingBase {
  def get: A = x
  def isEmpty = false
  ...
  override def pending(startTime: Long = startTime): Pot[A] = copy(x, startTime)
  override def fail(exception: Throwable): Pot[A] = FailedStale(x, exception)
}

object PendingStale {
  def make[A](a: A): URIO[Clock, PendingStale[A]] =
    Pot.currentTime.map(PendingStale(a, _))
}
...
sealed trait FailedBase {
  ...
  def isFailed = true
  def isUnavailable = false
  def state: PotState = PotState.PotFailed
}
...
final case class Failed(exception: Throwable)
  ...
  def isEmpty = true
  def isStale = false
  override def exceptionOption: Option[Throwable] = Some(exception)
...
final case class FailedStale[+A](x: A, exception: Throwable)
    extends Pot[A]
    with FailedBase {
  def get: A = x
  def isEmpty = false
  ...
  override def recoverWith[B >: A](
      f: PartialFunction[Throwable, Pot[B]]
  ): Pot[B] = {
    if (f isDefinedAt exception)
      f(exception)
    else
